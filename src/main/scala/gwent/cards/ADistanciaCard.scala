...
class ADistanciaCard (name: String, power: Int, habilidad: List[AplicaHabilidad]) extends AbstractUnitCard (name, power, habilidad){
  /**
   * Adds the ADistanciaCard to the corresponding section of the player's board, works by using double dispatch
   *
   * @param player The player to whom the ADistanciaCard belongs.
   * @param board  The game board where the ADistanciaCard will be added.
   */
